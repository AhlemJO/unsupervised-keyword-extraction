...
for counter,g in enumerate(gs):
    ...
    keywords['kc'].append(core_numbers)
    # weighted k-core
    ### fill the gaps (repeat the procedure used for k-core) ###
    core_numbers_w = core_dec(g,True)
    keywords['wkc'].append(core_numbers_w)
...
for counter,vector in enumerate(vectors_list):
    ...
    nonzero=[sublist for sublist in terms_weights if sublist[1]!=0.0 ]
